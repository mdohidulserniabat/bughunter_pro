...
```
bughunter_pro/
├── bughunter.sh          ← মেইন স্ক্রিপ্ট (এটা দিয়ে scan চালান)
├── install.sh            ← সব tool install করে (একবারই চালাতে হবে)
├── setup_keys.sh         ← API key সেভ করে (একবারই চালাতে হবে)
├── config.sh             ← API key template
└── modules/
    ├── 01_subdomain.sh   ← Subdomain enumeration (20+ sources)
    ├── 02_urls_js.sh     ← URL + JS analysis
    ├── 03_api_nuclei_web.sh  ← API + Nuclei + Web vulns
    ├── 04_sqli_secrets_recon.sh  ← SQLi + XXE + Secrets + Recon
    ├── 05_report.sh      ← HTML/TXT report generator
    └── 06_fixes.sh       ← Modern attacks + extra modules
```
...
এটা চালালে প্রতিটা key এর জন্য prompt আসবে।
**Blank রাখলে skip হবে** — সব key না থাকলেও চলবে।
...
Key গুলো `~/.bughunter/config.sh` এ সেভ হয়।
**পরের scan থেকে আর key দিতে হবে না।**
...
# Available modules:
# sub, url, js, api, nuclei, web, sqli, xxe,
